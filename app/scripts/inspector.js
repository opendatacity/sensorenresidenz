$(function () {
	...
	$('.floorplan').on('mouseenter', '.sensor', function (event) {
		...
		if (event.target.nodeName !== 'polygon'){
			if($(this).data('sensor') != undefined){
				$('.inhalt').remove();
				updateInspector($(this).data('sensor'));
			}else{
				...
				var muteId = $(this).attr('id');
				var key = muteId.split(" ")[0].toLowerCase();
				console.log(key);
				$find('.js-device-name').text(muteId);

				$('<div/>').addClass('inhalt')
					.text(sensorLabels[key])
					.appendTo('.sidebar');
	...
	$inspector.on('mousemove', '.axes', function (event) {
		graphs.sensorAxes($(this).parents('.js-device-sensor').data('sensor'), this, {
			x: event.offsetX,
			y: event.offsetY
		});
	});
