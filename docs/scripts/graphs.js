var graphs = (function () {
	...
	function sensorAxes (sensor, canvas, highlight) {
		...
		for (var h=0; h<=24; h++) {
			...
			if (labelHour) {
				ctx.fillText(h, s(x), s(y - .5 * blockSize));
			}
		}
		...
		while (date < endDate) {
			...
			if (labelDate) ctx.fillText(date.getDate(), s(x + 2), s(y*.8));
			...
			if (labelMonth) {
				...
				if (s(x + 4) + daySize.width + monthSize.width <= width) {
					...
					ctx.fillText(i18n('MMM'+date.getMonth()), s(x + 4) + daySize.width, s(y*.8));
					ctx.fillStyle = '#000';
				}
			}
			if (!highlight && day === 1) {
